...
class SRGAN():
    ...
    def Build_Generator(self):
        def Residual_Block(layer_input,filters,kernel_size):
            ...
            x=keras.layers.Conv2DTranspose(filters=filters,kernel_size=kernel_size,strides=1,padding='same')(layer_input)
            ...
            x=keras.layers.Conv2DTranspose(filters=filters,kernel_size=kernel_size,strides=1,padding='same')(x)
            x=keras.layers.BatchNormalization(momentum=0.8)(x)
        ...
        input_image=keras.layers.Input(shape=self.generator_image_shape,name='LR_Input')
        # x=keras.layers.Conv2D(filters=64,kernel_size=9,strides=1,padding='same')(input_image)
        x=keras.layers.Conv2DTranspose(filters=64,kernel_size=9,strides=1,padding='same')(input_image)
        x=keras.layers.PReLU()(x)
        # x=keras.layers.Activation('relu')(x)
        for _ in range(5):
            x=Residual_Block(x,64,3)
        # x=keras.layers.Conv2D(filters=64,kernel_size=3,strides=1,padding='same')(x)
        x=keras.layers.Conv2DTranspose(filters=64,kernel_size=3,strides=1,padding='same')(x)
        x=keras.layers.BatchNormalization(momentum=0.8)(x)
    ...
    def Build_Discriminator(self):
        inputs=keras.layers.Input(shape=self.discriminator_image_shape)
        kernel_size=5
    ...
    def Build_GAN(self):
        ...
        Ad_input=keras.layers.Input(shape=self.generator_image_shape)
