...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 31 17:02:51 2021
...
class Generator(tf.keras.Model):
    ...
    def call(self,inputs,training=None):
        x_layer=tf.nn.relu(self.Conv0(inputs))
        
        x=tf.nn.relu(self.bn10(self.Conv10(x_layer),training=training))
        x=self.bn11(self.Conv11(x),training=training)
        x_res_0=tf.keras.layers.add([x_layer,x])
        
        
        x=tf.nn.relu(self.bn20(self.Conv20(x_res_0),training=training))
        x=self.bn21(self.Conv21(x),training=training)
        x_res_1=tf.keras.layers.add([x_res_0,x])
        
        x=tf.nn.relu(self.bn30(self.Conv30(x_res_1),training=training))
        x=self.bn31(self.Conv31(x),training=training)
        x_res_2=tf.keras.layers.add([x_res_1,x])
        
        x=tf.nn.relu(self.bn40(self.Conv40(x_res_2),training=training))
        x=self.bn41(self.Conv41(x),training=training)
        x_res_3=tf.keras.layers.add([x_res_2,x])
        
        x=tf.nn.relu(self.bn50(self.Conv50(x_res_3),training=training))
