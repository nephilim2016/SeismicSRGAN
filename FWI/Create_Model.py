...
#Create Thin Model
def Overthrust_Model_half(xl,zl,CPML):
    vp=np.empty((xl+2*CPML+2*8,zl+2*CPML+2*8))
    vp_data=np.load('OverThrustClip.npy')
    
    vp_8=vp_data[::2,::2]
    vp[CPML+8:CPML+8+xl,CPML+8:CPML+8+zl]=vp_8
...
def Overthrust_Model(xl,zl,CPML):
    ...
    vp[CPML+8:CPML+8+xl,CPML+8:CPML+8+zl]=vp_data
    vp[:CPML+8,:]=vp[CPML+8,:]    #top
    vp[CPML+8+xl:,:]=vp[CPML+8+xl-1,:]    #bottom
    vp[:,:8+CPML]=vp[:,8+CPML].reshape((len(vp[:,8+CPML]),-1))    #left
    vp[:,8+CPML+zl:]=vp[:,8+CPML+zl-1].reshape((len(vp[:,8+CPML+zl:]),-1))    #right
...
#Create Thin Model
def Initial_Overthrust_Model(vp,sig):
...
def Marmousi_ii(xl,zl,CPML):
    ...
    mask=rho_data-1.01<0.1
    rho_data[mask]=1.8
    
    rho_1=rho_data[201:,5600:9300]
    vp_1=vp_data[201:,5600:9300]
    
    rho_8=skimage.transform.resize(rho_1,(xl,zl),mode='edge')
    vp_8=skimage.transform.resize(vp_1,(xl,zl),mode='edge')
